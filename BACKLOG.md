...
**2 tickets archived, 13 open.** This backlog opens with an architectural change: the crate is going to
...
| | ID | Ticket | Size | Blocked by |
|---|---|---|---|---|
| ☐ | **AG-012** | **Pin the torso+head baseline in a test.** *New ticket, and it goes first.* The numbers this whole backlog argues from — **7/12 watertight, 2/12 manifold, 4/12 collider-ready, 22 open cut edges** — are asserted by **nothing**. They are runtime output of `examples/fracture_cube.rs` and prose in `docs/research-brief.md:39-42`. Regress the fracture to 3/12 and 40 open edges and every test still passes; only the prose goes quietly stale. The only fixture CI actually locks is the convex `Cuboid`, which is exactly the case that was never broken.<br><br>That makes AG-001's pre-registered prediction unfalsifiable in the one direction that matters: we cannot show the numbers moved if nothing recorded where they started.<br>**Acceptance:** the torso+head fixture (two `Cuboid`s, `TARGET = 12`, `seed = 0x00C0_FFEE`) moves into `src/audit.rs` as a test asserting today's four numbers exactly, with a comment saying it is a **baseline, not a target**, naming AG-004 as the ticket that retires it. A change that moves these numbers must now say so out loud. | S | — |
...
| ☐ | **AG-006** | **Scope the fan-fold claim, and commit the fixture that breaks it.** `docs/isomesh-upstream-asks.md` §5 and `src/audit.rs`'s `known_defect_cap_fan_folds_on_a_non_convex_section` both state that a fan fold is *equivalent* to `inconsistently_oriented_edges > 0`. That equivalence needs **two** qualifiers, not one: it holds only for fans built with `push_cap_tri`'s per-triangle flip (`soup.rs:287`), **and** a fan that **winds twice without reversing** — a pentagram cap — folds with both counters reading zero.<br>**Acceptance:** a pentagram fixture committed and asserted (fold present, counters zero), and the claim restated as scoped rather than universal in both places.<br><br>*Related, and **half of it is false** — corrected in place rather than deleted, because the correction is the point:* `cap_side`'s fan apex **is** a vertex average rather than an area centroid (`soup.rs:323`), and that much stands. But the claim that `assemble_loops` "returns loops whose first vertex is duplicated at the end", double-weighting the apex, is **wrong**: `loop_v` starts as `vec![s0, s1]` and the walk breaks on `cur == s0` *before* pushing it again (`soup.rs:237-241`), so every vertex appears exactly once — which is also why `cap_side` closes the fan with `lp[(k + 1) % n]` (`soup.rs:327`). A duplicated first vertex would make that modulo wrap emit a degenerate final triangle.<br>Note the true half in the doc comment; do not fix it, because Phase 1 deletes the concern. | S | — |
